...
const Docs = ({ docs }) => {
  ...
  const deleteDoc = (id, docID) => {
    ...
    fetch(api_base_url + "/documents/deleteDoc", {
      mode: "cors",
      method: "POST",
      headers: {
        ...
        <div onClick={()=>{navigate(`/createDocs/${docs._id}`)}} className="left flex items-center gap-2">
          ...
          <div>
            ...
            <p className='text-[14px] text-[#808080]'>
              Created In : {new Date(docs.date).toDateString()} | Last Updated : {new Date(docs.lastUpdate).toDateString() } | Last Updated By: {docs.lastUpdatedBy} 
            </p>
          </div>
        </div>
